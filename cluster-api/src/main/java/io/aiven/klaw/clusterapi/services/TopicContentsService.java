...
public class TopicContentsService {
  ...
  public Map<Long, String> readEvents(
    ...
    if (offsetPosition.equals(CUSTOM_OFFSET_SELECTION) || offsetPosition.equals(RANGE_OFFSET_SELECTION)) {
    ...
    if (readMessagesType.equals("OFFSET_ID")) {
      for (TopicPartition tp : partitionsAssignment) {
        ...
        if (offsetPosition.equals(CUSTOM_OFFSET_SELECTION)) {
          ...
          newOffset = rangeOffsetsStart;//check with big small values
        } else {
    ...
    do {
      ...
      for (ConsumerRecord<String, String> record : consumerRecords) {
        ...
        if (offsetPosition.equals(RANGE_OFFSET_SELECTION) && (record.offset() >= rangeOffsetsEnd || eventMap.size() >= RANGE_MAX_RECORDS)) {
          exitLoop = true;
          break;
        }
      }
